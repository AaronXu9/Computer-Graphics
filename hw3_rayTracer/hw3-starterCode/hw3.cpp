...
//Sphere Intersection 
bool Ray::intersectSphere(const Sphere& sphere, Vector& intersection, double& dist) {
  Vector center(sphere.position[0], sphere.position[1], sphere.position[2]);
  Vector distance = origin - center;
  ...
  for (int j = 0; j < num_lights; ++j) {
    ...
    for (int k = 0; k < num_triangles; ++k) {
      ...
      if (shadow.intersectTriangle(triangles[k], shadow_intersection, distance)) {
        Vector dist_shadow = shadow_intersection - sphere_intersection;
        Vector dist_light = lightPos - sphere_intersection;
        // Make sure that the shadow intersection Vector does not pass the light
